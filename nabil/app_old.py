...
NAVY, NAVYLT = "#0f2744", "#1a3a6b"
ORANGE, ORANGEBG = "#f97316", "#fff7ed"
GREEN, RED, AMBER, GRAY, GRAYLT = "#16a34a", "#dc2626", "#d97706", "#64748b", "#f1f5f9"
...
[data-testid="stSidebar"] .stSelectbox>div>div{{background:#1e3a5f!important;border-color:#334155!important;color:white!important}}
[data-testid="stSidebar"] .stButton>button{{background:#1e3a5f!important;color:white!important;border:1px solid #334155!important}}
section[data-testid="stSidebar"] hr{{border-color:#334155!important}}
.block-container{{padding-top:1.5rem!important;padding-bottom:2rem!important}}

.hero{{background:linear-gradient(135deg,{NAVY} 0%,{NAVYLT} 100%);border-radius:16px;padding:32px 44px;margin-bottom:20px}}
.kpi-card{{background:white;border-radius:12px;padding:20px 16px;text-align:center;border:1px solid #e2e8f0;height:108px;display:flex;flex-direction:column;justify-content:center}}
.kpi-number{{font-size:2rem;font-weight:800;line-height:1;color:{NAVY}}}
.kpi-label{{font-size:0.7rem;color:{GRAY};text-transform:uppercase;letter-spacing:.06em;margin-top:5px}}
.kpi-delta{{font-size:0.76rem;margin-top:4px}}
.sec{{border-left:4px solid {ORANGE};padding-left:14px;margin:28px 0 12px 0}}
.sec-title{{font-size:1.05rem;font-weight:700;color:{NAVY};margin:0}}
.sec-sub{{font-size:0.8rem;color:{GRAY};margin:3px 0 0 0}}
.insight{{background:{ORANGEBG};border:1px solid #fed7aa;border-left:4px solid {ORANGE};border-radius:0 10px 10px 0;padding:12px 16px;margin-bottom:12px;font-size:0.86rem;color:#431407}}
.lcard{{background:white;border-radius:14px;padding:24px;margin-bottom:14px;border:1.5px solid #e2e8f0}}
.lcard-active{{border:2.5px solid {ORANGE}!important;box-shadow:0 4px 24px rgba(249,115,22,.18)}}
.lcard-locked{{opacity:.52}}
.tag{{display:inline-block;background:{GRAYLT};border-radius:20px;padding:3px 10px;font-size:0.74rem;color:#475569;margin:2px 2px 0 0}}
.tag-orange{{background:{ORANGEBG};color:#9a3412}}
.badge-live{{background:{ORANGEBG};color:#9a3412;border-radius:20px;padding:3px 10px;font-size:0.7rem;font-weight:700}}
.badge-soon{{background:{GRAYLT};color:#94a3b8;border-radius:20px;padding:3px 10px;font-size:0.7rem}}
.mod-card{{background:white;border-radius:14px;padding:22px;border:1.5px solid #e2e8f0;height:100%;cursor:pointer;transition:all .2s}}
.mod-card:hover{{border-color:{ORANGE};box-shadow:0 4px 16px rgba(249,115,22,.12)}}
.mod-icon{{font-size:1.8rem;margin-bottom:8px}}
.mod-title{{font-size:0.95rem;font-weight:700;color:{NAVY};margin:0 0 4px 0}}
.mod-desc{{font-size:0.78rem;color:{GRAY};margin:0 0 10px 0;line-height:1.4}}
.mod-stat{{font-size:1.4rem;font-weight:800;margin:8px 0 2px 0}}
.mod-statlbl{{font-size:0.68rem;color:{GRAY};text-transform:uppercase;letter-spacing:.04em}}
.kpi-tile{{border-radius:9px;padding:11px 14px;margin-bottom:8px}}
.metric-row{{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:0.82rem}}
.metric-row:last-child{{border-bottom:none}}
.flag-red{{background:#fef2f2;border:1px solid #fca5a5;border-left:4px solid {RED};border-radius:0 9px 9px 0;padding:10px 14px;margin-bottom:8px;font-size:0.82rem;color:#7f1d1d}}
.flag-amber{{background:#fffbeb;border:1px solid #fde68a;border-left:4px solid {AMBER};border-radius:0 9px 9px 0;padding:10px 14px;margin-bottom:8px;font-size:0.82rem;color:#713f12}}
.flag-green{{background:#f0fdf4;border:1px solid #bbf7d0;border-left:4px solid {GREEN};border-radius:0 9px 9px 0;padding:10px 14px;margin-bottom:8px;font-size:0.82rem;color:#14532d}}
</style>""", unsafe_allow_html=True)

# ── DATA ──────────────────────────────────────────────────────
@st.cache_data
def load_data():
    features = pd.read_csv(BASE / "features_dax15.csv")
    model    = pd.read_csv(BASE / "model_results.csv")
    coefs    = pd.read_csv(BASE / "model_coefs.csv")
    anomaly_u = pd.read_csv(BASE / "anomaly_scores_universe.csv")
    merge_cols = ["company_shortname","year","pred_comp","pred_comp_low",
                  "pred_comp_high","actual_vs_expected_pct","benchmark_signal"]
    df = features.merge(model[merge_cols], on=["company_shortname","year"], how="left")
    esg = pd.DataFrame()
    p = BASE / "esg_dashboard_data.csv"
    if p.exists():
        esg = pd.read_csv(p)
    return df, coefs, anomaly_u, esg

df, coefs, anomaly_u, esg = load_data()
COMPANIES = sorted(esg["company_shortname"].unique()) if len(esg) > 0 else sorted(df["company_shortname"].unique())
MODEL_COS = set(df["company_shortname"].unique())
YEARS_M   = sorted(df["year"].unique())
...
def kpi_html(number, label, delta, num_color=None, delta_color=GRAY):
...
def sidebar_nav(back_screen, back_label="← Zurück"):
    with st.sidebar:
        ...
        yr_idx = YEARS_M.index(st.session_state.year) if st.session_state.year in YEARS_M else len(YEARS_M)-1
        yr = st.selectbox("", YEARS_M, index=yr_idx, label_visibility="collapsed", key="sb_yr")
...
def show_landing():
    ...
    st.markdown(f"""<div class="hero">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px;">
            <div>
                <div style="font-size:.72rem;color:#94a3b8;text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px;">TUM Science Hackathon 2026</div>
                <div style="font-size:2.4rem;font-weight:900;color:white;letter-spacing:-1px;line-height:1.1;">ExComp</div>
                <div style="font-size:.95rem;color:#94a3b8;margin-top:8px;">AI-Powered Executive Compensation Benchmarking &amp; Red Flag Detection</div>
                <div style="margin-top:12px;font-size:.84rem;color:#cbd5e1;">
                    15 Jahre Vergütungsdaten · Predictive Modeling · Anomalie-Erkennung · ESG-Integration
                </div>
            </div>
            <div style="display:flex;gap:28px;flex-wrap:wrap;">
                <div style="text-align:center;"><div style="font-size:2.2rem;font-weight:900;color:{ORANGE};">16/43</div><div style="font-size:.7rem;color:#94a3b8;text-transform:uppercase;">ESG in Exec-Pay</div></div>
                <div style="text-align:center;"><div style="font-size:2.2rem;font-weight:900;color:white;">0.71</div><div style="font-size:.7rem;color:#94a3b8;text-transform:uppercase;">Modell R²</div></div>
                <div style="text-align:center;"><div style="font-size:2.2rem;font-weight:900;color:white;">74x</div><div style="font-size:.7rem;color:#94a3b8;text-transform:uppercase;">Max Pay Ratio</div></div>
            </div>
        </div>
    </div>""", unsafe_allow_html=True)

    st.markdown("""<div style="text-align:center;margin:4px 0 20px 0;">
        <div style="font-size:.98rem;font-weight:700;color:#0f2744;">Wähle deinen Stakeholder-Blickwinkel</div>
        <div style="font-size:.82rem;color:#64748b;margin-top:3px;">Vier Nutzergruppen — eine vollständig verfügbare Analyse</div>
    </div>""", unsafe_allow_html=True)
...
def show_overview():
    ...
    # ── Hero ──
    aep      = float(co_yr["actual_vs_expected_pct"].iloc[0]) if len(co_yr) > 0 and co_yr["actual_vs_expected_pct"].notna().any() else None
    gov_sc   = float(co_df["anomaly_score_pct"].mean()) if in_model and len(co_df) > 0 else None
    peer_pct = float(co_yr["peer_pct_total_comp"].iloc[0]) if len(co_yr) > 0 and "peer_pct_total_comp" in co_yr.columns and co_yr["peer_pct_total_comp"].notna().any() else None
    is_anom  = bool(co_yr["is_anomaly"].iloc[0]) if len(co_yr) > 0 and "is_anomaly" in co_yr.columns and co_yr["is_anomaly"].notna().any() else None
    sti_esg  = last(co_esg["sti_esg_share"]) if len(co_esg) > 0 else None
    lti_esg  = last(co_esg["lti_esg_share"]) if len(co_esg) > 0 else None
    esg_tot  = (sti_esg or 0) + (lti_esg or 0)
    total_c  = float(co_yr["total_comp_bt"].iloc[0]) if len(co_yr) > 0 and co_yr["total_comp_bt"].notna().any() else None
    ...
    st.markdown(f"""<div class="hero">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
            <div>
                ...
                <div style="font-size:.84rem;color:#94a3b8;">DAX · Vergütungsanalyse {sel_year} · Wähle ein Modul um tiefer einzutauchen</div>
            </div>
    ...
    st.markdown('<div class="sec"><div class="sec-title">Module — wähle eine Analyseebene</div><div class="sec-sub">Jedes Modul deckt einen der fünf Hackathon-Anforderungsbereiche ab</div></div>', unsafe_allow_html=True)
    ...
    MODULES = [
        ...
        ("anomaly", "🔍", "Anomalie-Detektor",
         ...
         f"Status {sel_year}"),
        ("esg", "🌱", "ESG-Rating",
         "Verknüpft die Vergütung ESG-Ziele? CSRD-Compliance, Gender Equity & CEO/Worker Pay Ratio.",
    ...
    for i, (key, icon, title, desc, stat, statlbl) in enumerate(MODULES):
        ...
        with col:
            st.markdown(f"""<div class="mod-card">
                ...
                <div style="border-top:1px solid #f1f5f9;padding-top:10px;margin-top:8px;">
                    <div class="mod-stat" style="color:{ORANGE};">{stat}</div>
...
def show_historical():
    ...
    with col_l:
        st.markdown('<div class="sec"><div class="sec-title">Vergütungstrend vs. DAX-Durchschnitt</div><div class="sec-sub">Gesamtvergütung Vorstand (€ Tsd.) — blaue Linie = ausgewähltes Unternehmen</div></div>', unsafe_allow_html=True)
    ...
    if in_model and len(co_df) > 0 and all(c in co_df.columns for c in ["fixed_pct","sti_pct","lti_pct"]):
        st.markdown('<div class="sec"><div class="sec-title">Vergütungsstruktur-Evolution (Fix / STI / LTI)</div><div class="sec-sub">Wie hat sich der Mix aus fixer und variabler Vergütung verändert?</div></div>', unsafe_allow_html=True)
        st.markdown("""<div class="insight">
...
def show_prediction():
    ...
    module_header("🤖", "Prognosemodell", "OLS-Regressionsmodell · R²=0.71 · Expected Pay vs. Actual Pay", sel_co, sel_year)
    ...
    with col_l:
        st.markdown('<div class="sec"><div class="sec-title">Tatsächlich vs. Modell-Erwartung</div><div class="sec-sub">Oranger Bereich = 80% Erwartungsintervall · Rote Linie = Tatsächliche Vergütung</div></div>', unsafe_allow_html=True)
        if in_model and len(co_df) > 0 and co_df["pred_comp"].notna().any():
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=list(co_df["year"])+list(co_df["year"])[::-1],
                y=list(co_df["pred_comp_high"])+list(co_df["pred_comp_low"])[::-1],
                fill="toself", fillcolor="rgba(249,115,22,.10)",
                line=dict(color="rgba(0,0,0,0)"), name="80% Erwartungsband"))
            fig.add_trace(go.Scatter(x=co_df["year"], y=co_df["pred_comp"],
            ...
            # Flag overpaid years
            over = co_df[co_df["actual_vs_expected_pct"] > 40]
            if len(over) > 0:
            ...
            st.info(f"Prognosemodell-Daten nur für Kerngruppe (15 Unternehmen) verfügbar. {sel_co} ist nicht enthalten.")

    with col_r:
        st.markdown('<div class="sec"><div class="sec-title">Abweichung vom Modell</div><div class="sec-sub">Positiv = überbezahlt, Negativ = unterbezahlt</div></div>', unsafe_allow_html=True)
        if in_model and co_df["actual_vs_expected_pct"].notna().any():
            aep_series = co_df.dropna(subset=["actual_vs_expected_pct"])
            ...
            co_yr = df[(df["company_shortname"]==sel_co) & (df["year"]==sel_year)]
            aep_now = float(co_yr["actual_vs_expected_pct"].iloc[0]) if len(co_yr)>0 and co_yr["actual_vs_expected_pct"].notna().any() else None
            ...
            st.info("Nicht in Modell-Kerngruppe.")

    # Feature Importance
    st.markdown('<div class="sec"><div class="sec-title">Was treibt die Vergütung? — Modell-Koeffizienten</div><div class="sec-sub">Wie stark beeinflusst jeder Faktor das Vergütungsniveau? (Effekt in %)</div></div>', unsafe_allow_html=True)
    st.markdown("""<div class="insight">
...
def show_governance():
    ...
    with col_l:
        st.markdown('<div class="sec"><div class="sec-title">Governance Risk Heatmap — DAX Kerngruppe</div><div class="sec-sub">Rot = hohes Risiko · Grün = niedriges Risiko · Orangener Rahmen = ausgewähltes Unternehmen</div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="sec"><div class="sec-title">Schlechte-Zeiten-Ereignisse — DAX Kerngruppe</div><div class="sec-sub">Vergütung stieg, obwohl EBIT und Headcount sanken — klarer Pay-for-Performance-Verstoß</div></div>', unsafe_allow_html=True)
...
def show_peer():
    ...
    sel_co, sel_year = st.session_state.company, st.session_state.year
    in_model = sel_co in MODEL_COS
    year_data = df[df["year"] == sel_year].sort_values("total_comp_bt", ascending=False)
    co_df = df[df["company_shortname"] == sel_co]

    module_header("👥", "Peer Benchmarking", f"DAX-Kerngruppe Vergleich · {sel_year} · Sektorpositioning", sel_co, sel_year)
    ...
    if len(year_data) > 0:
        ...
        co_yr = year_data[year_data["company_shortname"]==sel_co]
        rank = int(co_yr["peer_rank_total_comp"].iloc[0]) if len(co_yr)>0 and "peer_rank_total_comp" in co_yr.columns and co_yr["peer_rank_total_comp"].notna().any() else None
        pctile = float(co_yr["peer_pct_total_comp"].iloc[0]) if len(co_yr)>0 and "peer_pct_total_comp" in co_yr.columns and co_yr["peer_pct_total_comp"].notna().any() else None
        aep = float(co_yr["actual_vs_expected_pct"].iloc[0]) if len(co_yr)>0 and co_yr["actual_vs_expected_pct"].notna().any() else None
        total = float(co_yr["total_comp_bt"].iloc[0]) if len(co_yr)>0 and co_yr["total_comp_bt"].notna().any() else None
    ...
    st.markdown('<div class="sec"><div class="sec-title">Vergütungsvergleich — alle DAX-Unternehmen</div><div class="sec-sub">Balken = tatsächliche Vergütung · Rauten = Modell-Erwartungsband · Farbe = Benchmark-Signal</div></div>', unsafe_allow_html=True)
    ...
    if in_model and "sector" in df.columns:
        ...
        if co_sector:
            st.markdown(f'<div class="sec"><div class="sec-title">Sektorvergleich: {co_sector}</div><div class="sec-sub">Wie steht {sel_co} vs. Unternehmen im gleichen Sektor?</div></div>', unsafe_allow_html=True)
...
def show_anomaly():
    ...
    with col_l:
        st.markdown('<div class="sec"><div class="sec-title">Anomalie-Score vs. Vergütungshöhe — Universum</div><div class="sec-sub">Alle Unternehmen im Datensatz · Orange markiert = ausgewählt · Rot = flagged als Anomalie</div></div>', unsafe_allow_html=True)
...
def show_esg():
    ...
    module_header("🌱", "ESG-Rating", "ESG-Integration in Vergütung · CSRD-Compliance · Gender Equity · Pay Ratio", sel_co, sel_year)

    sti_esg = last(co_esg["sti_esg_share"]) if len(co_esg) > 0 else None
    lti_esg = last(co_esg["lti_esg_share"]) if len(co_esg) > 0 else None
    fem_pct = last(co_esg["female_pct"])     if len(co_esg) > 0 else None
    w_ratio = last(co_esg["ceo_worker_ratio"]) if len(co_esg) > 0 else None
    esg_tot = (sti_esg or 0) + (lti_esg or 0)
    ...
    # DAX Bubble Chart
    st.markdown('<div class="sec"><div class="sec-title">DAX-Überblick: Wer integriert ESG wirklich?</div><div class="sec-sub">X/Y = ESG-Anteil STI/LTI · Punktgröße = Ø Vergütung · Orange = mit ESG · Grau = ohne ESG</div></div>', unsafe_allow_html=True)
    ...
    with col_kpi:
        st.markdown(f"**KPI-Texte aus dem Vergütungsbericht — {sel_co}**")
    ...
    with col_csrd:
        st.markdown("**CSRD-Compliance-Check**")
        OPACITY = {"BMW","Volkswagen","Mercedes-Benz"}
        co_kpi_rows2 = co_esg[co_esg[["stipi1","stipi2","ltipi1","ltipi2"]].notna().any(axis=1)] if len(co_esg)>0 else pd.DataFrame()
        checks = [
            ("ESG-Ziel in STI verankert",           sti_esg is not None and sti_esg > 0),
            ("ESG-Ziel in LTI verankert",           lti_esg is not None and lti_esg > 0),
            ("KPI-Text öffentlich verifizierbar",   len(co_kpi_rows2)>0 and sel_co not in OPACITY),
    ...
    # Pay Equity
    st.markdown('<div class="sec"><div class="sec-title">Pay Equity — Gender &amp; CEO/Worker Ratio</div><div class="sec-sub">Frauenanteil vs. ARUG II-Ziel · Einkommensungleichheit im DAX-Vergleich</div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div style="text-align:center;color:#94a3b8;font-size:.74rem;padding:16px 0 4px 0;border-top:1px solid #e2e8f0;margin-top:12px;">ExComp · TUM Science Hackathon 2026 · ORBIS/Bureau van Dijk · DGAP · 43 DAX-Unternehmen · 2006–2024</div>', unsafe_allow_html=True)
